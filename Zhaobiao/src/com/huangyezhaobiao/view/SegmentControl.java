    ...
    public void onDraw(Canvas canvas){
        ...
        if(mTexts != null && mTexts.length > 0){
            for(int i = 0; i < mTexts.length; i++){
                ...
                //为了使字体可以居中显示所以加上的内容
                float v = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, getContext().getResources().getDisplayMetrics());
                //draw texts
                canvas.drawText(mTexts[i], mCacheBounds[i].left + (mSingleChildWidth - mTextBounds[i].width()) / 2, mCacheBounds[i].top + ((mSingleChildHeight + mTextBounds[i].height()) / 2)-v, mPaint);
            }
        }
